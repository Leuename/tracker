...
| `npm run schedule` | Generate this month's recurring payables and report what is overdue. Idempotent; `-- --dry-run` writes nothing. Run daily at 22:00 UTC by `.github/workflows/schedule.yml`. Needs `SCHEDULE_EMAIL`/`SCHEDULE_PASSWORD`, named in `.env.example`. |
