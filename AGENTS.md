...
| `npm test` | `node --test src/logic.test.js src/rows.test.js src/errors.test.js scripts/rewind-plan.test.js`. 53 assertions. Offline. No test framework. |
| `npm run e2e` | 27 Playwright specs. **Run it as `npx playwright test --workers=1`**: `fullyParallel: false` only serialises within a file, so the default four workers race each other on the one shared ledger and the suite fails about as often as it passes. `workers: 1` is proposed, not applied. They drive the UI and read Postgres back, so they need `E2E_EMAIL` and `E2E_PASSWORD`; `E2E_BASE_URL` points them at a deployment. They write to the shared ledger and sweep every `E2E-` tag before and after. Without credentials they skip and exit 0; set `E2E_REQUIRE_CREDENTIALS=1` to make that a failure instead. |
| `npm run security` | 47-check probe: anonymous access, forged tokens, schema exposure, filter injection, mass assignment, audit-log tamper resistance, role integrity, storage, bundle secrets, sign-up, deployment headers. Checks named in `DEFERRED` still run and print, tagged `DEFER`, but do not fail the suite ([D26](docs/Decisions.md)); the list is empty and all 47 pass. |
| `npm run schedule` | Generate this month's recurring payables and report what is overdue. Idempotent; `-- --dry-run` writes nothing. Run daily by `.github/workflows/schedule.yml`. Needs `SCHEDULE_EMAIL`/`SCHEDULE_PASSWORD`, which `.env.example` does not yet name. |
| `npm run rewind -- --since <ISO>` | Reconstruct the ledger as it stood at any second, from `audit_log` ([D32](docs/Decisions.md)). Writes nothing to the database: it prints the plan and emits a `.sql` file to apply as `postgres`. Generated plans are gitignored. |
