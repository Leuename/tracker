...
| `npm test` | `node --test src/logic.test.js src/rows.test.js src/errors.test.js`. 43 assertions. Offline. No test framework. |
...
| `npm run schedule` | Generate this month's recurring payables and report what is overdue. Idempotent; `-- --dry-run` writes nothing. Run daily by `.github/workflows/schedule.yml`. |
