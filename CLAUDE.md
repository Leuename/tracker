...
| `npm test` | `node --test src/logic.test.js src/rows.test.js src/pending.test.js src/masterlist.test.js src/queries.test.js src/errors.test.js scripts/rewind-plan.test.js scripts/schedule-plan.test.js scripts/backup-plan.test.js e2e/held.test.js e2e/network-preflight.test.js`. 191 assertions across 11 files. Offline. No test framework. The preflight tests pin the **failure** paths a live e2e run never reaches — a reachable-but-broken deployment must fail the gate, not clear it. |
| `npm run e2e` | 50 Playwright tests in 3 files — a `setup` project that signs in twice, then 48 specs. The new occurrence-identity spec skips safely before phase 1 and has **not run against the hosted column**. `workers: 1` **is applied** in `playwright.config.js:43`, because `fullyParallel: false` only serialises within a file and the default four workers raced each other on the one shared ledger. A setup project saves `storageState`, so 46 of the 48 no longer type the password; the two that exercise the sign-in form still do. Session-mutating specs are **ordered**, and that ordering is load-bearing ([D41](docs/Decisions.md)). They drive the UI and read Postgres back, so they need `E2E_EMAIL` and `E2E_PASSWORD`; `E2E_BASE_URL` points them at a deployment. They write to the shared ledger and sweep every `E2E-` tag before and after. Four specs also change the **owner’s live config** — three settings and the shared company/category lists; each calls `hold()` first, which records what was there into `settings.__e2eHeld` so a killed process is recoverable, and both `beforeAll` and every `finally` call `releaseHeld()` ([D71](docs/Decisions.md), [D72](docs/Decisions.md)). Without credentials they skip and exit 0; set `E2E_REQUIRE_CREDENTIALS=1` to make that a failure instead. |
| `npm run security` | 57-check probe: anonymous access, forged tokens, schema exposure, filter injection, mass assignment, audit-log tamper resistance, role integrity, storage, bundle secrets, sign-up, deployment headers. Checks named in `DEFERRED` still run and print, tagged `DEFER`, but do not fail the suite ([D26](docs/Decisions.md)). Run with `OCCURRENCE_IDENTITY_PHASE=1` before and after phase 1: the identity check may defer only while phase 1 is unapplied or its old-bundle compatibility is being verified; once the column exists it requires exact `42501` for `occurrence_due` while `src` remains updateable. Set `OCCURRENCE_IDENTITY_PHASE=2` only after phase 2: the check is removed from `DEFERRED`, requires exact `42501` for both, and any failure is fatal/nonzero ([D79](docs/Decisions.md)). |
| `npm run schedule` | Generate this month's recurring payables and report what is overdue. The working tree implements immutable generated-occurrence identity ([D79](docs/Decisions.md)), but its two migrations are **unapplied**: apply phase 1, deploy the identity-aware app, then apply phase 2. Until that owner-gated rollout, production still has D63's `(src, due)` backstop and the current deployment. `-- --dry-run` writes nothing. Run daily at 22:00 UTC by `.github/workflows/schedule.yml`. Needs `SCHEDULE_EMAIL`/`SCHEDULE_PASSWORD`, named in `.env.example`. |
...
| `npm run rewind -- --since <ISO>` | Reconstruct the ledger as it stood at any second, from `audit_log` ([D32](docs/Decisions.md)). Writes nothing to the database: it prints the plan and emits a `.sql` file to apply as `postgres`. Once occurrence identity exists, it refuses linked pre-migration `txns` images without `occurrence_due`; use the pre-migration schema or an explicit owner-approved mapping ([D79](docs/Decisions.md)). Generated plans are gitignored. |
