...
| `npm test` | `node --test src/logic.test.js src/rows.test.js`. Offline. No test framework. |
| `npm run e2e` | 24 Playwright specs. They drive the UI and read Postgres back, so they need `E2E_EMAIL` and `E2E_PASSWORD`; `E2E_BASE_URL` points them at a deployment. They write to the shared ledger and sweep every `E2E-` tag before and after. |
| `npm run security` | 33-check probe: anonymous access, forged tokens, schema exposure, filter injection, mass assignment, storage, bundle secrets, sign-up, deployment headers. |
...
The repository root is this directory, pushed to `Leuename/tracker` (private). **Vercel's Root Directory must be set to `apps/web`**, because that is where the only package manifest lives; without it every deploy fails looking for a manifest at the top. There is still no repository-root manifest, workspace declaration, task runner, or CI workflow, so nothing runs from the repository root. No formatter, linter, or type checker is configured. The `dc-runtime` build text inside generated `support.js` points to absent source and is not runnable.

A handoff, recap, or continuation package is incomplete without a `## Resume prompts` section: copy-pasteable prompts that each name the document's exact path and instruct the next session to verify state before acting. [handoff/Session Continuation Package 2026-09-01.md](handoff/2026-09-01%20Session%20Continuation%20Package.md) is the current one and the reference shape.
