...
export function StoreProvider({ children }) {
  ...
  //
  // The effect keys on the config's VALUE, not on `state`. It used to depend on
  // `state`, and `reduce` returns a new object every time, so anything that
  // touched state re-ran it — including `flash`, which is how `save` reports a
  // failed write. Combined with the baseline rollback that closed a circle:
  // write fails, toast sets state, effect re-runs, same write re-issued. 79
  // writes and 79 toasts from one toggle in 500ms. A toast does not change the
  // config, so keying on the config cannot re-trigger a write.
