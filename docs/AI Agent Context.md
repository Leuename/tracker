...
- **briefed-to:** [Open Items Brief for Codex](../handoff/2026-09-04%20Open%20Items%20Brief%20for%20Codex.md) is the current brief for an external agent: **only what is still open**, with exact relative paths and no wikilinks, because the intended reader cannot resolve them. It reuses the R/F/N/Q ids rather than renumbering. **blocks:** two of its items need a `psql` connection string nobody has yet supplied.
